...
TEST_CASE("test look fo word with spaces") {
    string text = "dont worry be happy ";
...
TEST_CASE("look for substring") {
     string text = "hello big world ";
    CHECK_THROWS(find(text, "llo"));  // check that some exception is thrown
    CHECK_THROWS(find(text, "orl"));
    CHECK_THROWS(find(text, "g"));
}